...
serve(async (req) => {
  ...
  // LOGIN PAGE
  if (!currentUser) {
    return new Response(`<!DOCTYPE html><html><head><title>Welcome</title>${commonHead}</head><body class="h-screen flex items-center justify-center px-4 bg-slate-900">${splashHTML}${loaderHTML}<div class="glass-panel w-full max-w-sm p-8 rounded-3xl text-center"><div class="w-16 h-16 bg-blue-600 rounded-xl mx-auto mb-4 flex items-center justify-center shadow-lg shadow-blue-500/30"><i class="fas fa-chart-line text-2xl text-white"></i></div><h1 class="text-2xl font-bold text-white mb-1">Welcome Back</h1><p class="text-slate-400 text-sm mb-8">Sign in to continue</p><div class="flex justify-center mb-6 bg-slate-800/50 p-1 rounded-xl"><button onclick="showLogin()" id="tabLogin" class="w-1/2 py-2 rounded-lg font-bold text-sm bg-blue-600 text-white shadow-md">Login</button><button onclick="showRegister()" id="tabReg" class="w-1/2 py-2 rounded-lg font-bold text-sm text-slate-400">Register</button></div><form id="loginForm" action="/login" method="POST" onsubmit="showLoader()"><div class="space-y-4"><input name="username" placeholder="Username" class="w-full bg-slate-800/50 border border-slate-700 text-white p-3 rounded-xl outline-none focus:border-blue-500 transition" required><input type="password" name="password" placeholder="Password" class="w-full bg-slate-800/50 border border-slate-700 text-white p-3 rounded-xl outline-none focus:border-blue-500 transition" required><label class="flex items-center gap-2 text-xs text-slate-400 cursor-pointer"><input type="checkbox" name="remember" checked class="rounded bg-slate-700 border-slate-600 text-blue-600"> Remember Me (15 Days)</label><button class="w-full btn-primary py-3.5 rounded-xl font-bold mt-2">SIGN IN</button></div></form><form id="regForm" action="/register" method="POST" class="hidden" onsubmit="showLoader()"><div class="space-y-4"><input name="username" placeholder="Choose Username" class="w-full bg-slate-800/50 border border-slate-700 text-white p-3 rounded-xl outline-none focus:border-blue-500 transition" required><input type="password" name="password" placeholder="Choose Password" class="w-full bg-slate-800/50 border border-slate-700 text-white p-3 rounded-xl outline-none focus:border-blue-500 transition" required><label class="flex items-center gap-2 text-xs text-slate-400 cursor-pointer"><input type="checkbox" name="remember" checked class="rounded bg-slate-700 border-slate-600 text-blue-600"> Remember Me (15 Days)</label><button class="w-full btn-primary py-3.5 rounded-xl font-bold mt-2">CREATE ACCOUNT</button></div></form></div><script>const p=new URLSearchParams(window.location.search);if(p.get('error'))Toast.fire({icon:'error',title:p.get('error')});function showLogin(){document.getElementById('loginForm').classList.remove('hidden');document.getElementById('regForm').classList.add('hidden');const l=document.getElementById('tabLogin'),r=document.getElementById('tabReg');l.className='w-1/2 py-2 rounded-lg font-bold text-sm bg-blue-600 text-white shadow-md';r.className='w-1/2 py-2 rounded-lg font-bold text-sm text-slate-400';}function showRegister(){document.getElementById('loginForm').classList.add('hidden');document.getElementById('regForm').classList.remove('hidden');const l=document.getElementById('tabLogin'),r=document.getElementById('tabReg');r.className='w-1/2 py-2 rounded-lg font-bold text-sm bg-blue-600 text-white shadow-md';l.className='w-1/2 py-2 rounded-lg font-bold text-sm text-slate-400';}</script></body></html>`, { headers: { "content-type": "text/html; charset=utf-8" } });
  }
  ...
  if (page === "/history") {
      ...
      content = `
        <div class="p-4">
            ...
            <div class="glass-card rounded-xl overflow-hidden">
                <div class="grid grid-cols-3 bg-slate-800/50 p-3 text-[10px] font-bold text-slate-400 text-center tracking-wider"><div>DATE</div><div>12:01 PM</div><div>04:30 PM</div></div>
                ${historyList.map((h: any) => `<div class="grid grid-cols-3 p-3 text-center border-t border-slate-700/50 items-center"><div class="text-xs text-slate-300 font-mono">${h.date}</div><div class="font-bold text-blue-400 text-lg">${h.morning}</div><div class="font-bold text-purple-400 text-lg">${h.evening}</div></div>`).join('')}
      ...
      if (isAdmin) {
          ...
          adminStats = `<div class="glass-panel p-3 rounded-xl mb-4"><h3 class="text-xs font-bold text-slate-400 mb-2 uppercase">Today's Overview</h3><div class="grid grid-cols-3 gap-2"><div class="bg-green-500/10 p-2 rounded-lg text-center border border-green-500/20"><div class="text-[10px] text-green-400 font-bold">SALE</div><div class="font-bold text-sm">${sale.toLocaleString()}</div></div><div class="bg-red-500/10 p-2 rounded-lg text-center border border-red-500/20"><div class="text-[10px] text-red-400 font-bold">PAYOUT</div><div class="font-bold text-sm">${payout.toLocaleString()}</div></div><div class="bg-blue-500/10 p-2 rounded-lg text-center border border-blue-500/20"><div class="text-[10px] text-blue-400 font-bold">PROFIT</div><div class="font-bold text-sm">${(sale-payout).toLocaleString()}</div></div></div></div>`;
      }

      content = `
        <div class="bg-gradient-to-b from-blue-900 to-slate-900 p-6 pb-10 rounded-b-[40px] shadow-2xl text-center relative mb-4 border-b border-white/5">
        ...
        <div class="px-4 space-y-4 pb-6">
            ...
            <div class="glass-card p-4 rounded-xl">
                ...
                <form action="/admin/topup" method="POST" onsubmit="showLoader()" class="flex gap-2 mb-3"><input name="username" placeholder="User" class="w-1/3 bg-slate-800 border border-slate-700 rounded-lg p-2 text-xs text-white focus:border-blue-500 outline-none"><input name="amount" placeholder="Amount" type="number" class="w-1/3 bg-slate-800 border border-slate-700 rounded-lg p-2 text-xs text-white focus:border-blue-500 outline-none"><button class="bg-green-600 hover:bg-green-700 text-white w-1/3 rounded-lg text-xs font-bold transition">TOPUP</button></form>
                <form action="/admin/payout" method="POST" onsubmit="showLoader()" class="flex gap-2 mb-3"><select name="session" class="w-1/3 bg-slate-800 border border-slate-700 rounded-lg p-2 text-xs text-white outline-none"><option value="MORNING">12:01</option><option value="EVENING">04:30</option></select><input name="win_number" placeholder="Win No" class="w-1/3 bg-slate-800 border border-slate-700 rounded-lg p-2 text-center text-xs text-white focus:border-blue-500 outline-none"><button class="bg-red-600 hover:bg-red-700 text-white w-1/3 rounded-lg text-xs font-bold transition">PAYOUT</button></form>
                <form action="/admin/contact" method="POST" onsubmit="showLoader()" class="grid grid-cols-2 gap-2 mb-3"><input name="kpay_no" placeholder="KPay No" class="bg-slate-800 border border-slate-700 rounded-lg p-2 text-xs text-white"><input name="wave_no" placeholder="Wave No" class="bg-slate-800 border border-slate-700 rounded-lg p-2 text-xs text-white"><button class="col-span-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg py-2 text-xs font-bold transition">Update Contact</button></form>
                <form action="/admin/tip" method="POST" onsubmit="showLoader()" class="flex gap-2"><input name="tip" placeholder="Lucky Tip Text" class="flex-1 bg-slate-800 border border-slate-700 rounded-lg p-2 text-xs text-white focus:border-blue-500 outline-none"><button class="bg-purple-600 hover:bg-purple-700 text-white px-4 rounded-lg text-xs font-bold transition">TIP</button></form>
                <form action="/admin/add_history" method="POST" onsubmit="showLoader()" class="flex gap-2 mt-3"><input name="date" type="date" class="w-1/3 bg-slate-800 border-slate-700 rounded p-2 text-xs text-white"><input name="morning" placeholder="M" class="w-1/4 bg-slate-800 border-slate-700 rounded p-2 text-center text-xs text-white"><input name="evening" placeholder="E" class="w-1/4 bg-slate-800 border-slate-700 rounded p-2 text-center text-xs text-white"><button class="bg-gray-600 text-white w-1/6 rounded text-xs">Save</button></form>
            </div>` : ''}

            <div class="glass-card p-4 rounded-xl">
                <h3 class="text-xs font-bold text-slate-400 mb-3 uppercase tracking-wider">Security</h3>
                <form action="/change_password" method="POST" onsubmit="showLoader()" class="flex gap-2"><input type="password" name="new_password" placeholder="New Password" class="flex-1 bg-slate-800 border border-slate-700 rounded-lg p-2 text-sm text-white focus:border-blue-500 outline-none" required><button class="bg-slate-700 hover:bg-slate-600 text-white px-4 py-2 rounded-lg text-xs font-bold transition">CHANGE</button></form>
            </div>
      ...
      content = `
        <div class="flex justify-between items-center px-5 py-4 sticky top-0 bg-slate-900/90 backdrop-blur-md z-40 border-b border-white/5">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-full bg-slate-800 overflow-hidden border border-slate-600">${avatar ? `<img src="${avatar}" class="w-full h-full object-cover">` : `<i class="fas fa-user text-gray-400 m-2"></i>`}</div>
                <div><div class="text-[10px] text-slate-400 font-bold uppercase">Welcome</div><div class="text-sm font-bold text-white leading-none">${currentUser}</div></div>
            </div>
            <div class="bg-gradient-to-r from-slate-800 to-slate-900 px-3 py-1.5 rounded-lg border border-slate-700 flex items-center gap-2 shadow-sm">
        ...
        <div class="p-4">
            <div class="card-gradient rounded-2xl p-6 text-center shadow-lg relative overflow-hidden mb-5 border border-white/10">
            ...
            ${dailyTip ? `<div class="mb-5 relative overflow-hidden rounded-xl shadow-lg transform hover:scale-[1.02] transition duration-300"><div class="absolute inset-0 bg-gradient-to-r from-cyan-500 via-blue-500 to-indigo-600 animate-gradient-x"></div><div class="relative bg-slate-900/90 m-[1px] rounded-xl p-3 text-center backdrop-blur-md"><span class="bg-blue-500/20 text-blue-300 border border-blue-500/30 text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wide mb-1 inline-block">VIP TIP</span><p class="text-lg font-bold text-white">${dailyTip}</p></div></div>` : ''}

            ${!isAdmin ? `<button onclick="openBetModal()" class="w-full btn-gold py-4 rounded-xl font-bold text-lg shadow-lg shadow-orange-500/20 mb-6 active:scale-95 transition flex items-center justify-center gap-2"><i class="fas fa-plus-circle"></i> ထိုးမည် (Bet)</button>` : ''}

            <div class="grid grid-cols-2 gap-3 mb-6">
                <div class="glass-card p-4 rounded-xl flex flex-col items-center relative overflow-hidden group"><div class="absolute top-0 left-0 w-full h-1 bg-yellow-500/50"></div><div class="text-xs text-slate-400 font-bold mb-1">12:01 PM</div><div id="res_12" class="text-3xl font-bold text-white group-hover:text-gold transition">--</div><div id="set_12" class="text-[10px] text-slate-500 mt-1 font-mono">SET: --</div></div>
